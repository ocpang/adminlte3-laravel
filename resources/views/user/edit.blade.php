									...
									<div class="col-xl-12">
										<div class="row justify-content-center">
											<div class="col-xl-9">
												...
												@endif
												<form id="form_data" role="form" action="{{ route('user.save', $user->id) }}" method="POST">
													@csrf
													@method('POST')
													<input name="id" id="id_data" value="{{$user->id}}" type="hidden">
																...
																<select class="form-control select2" name="customer_id" id="customer_id">
																	<option value="">Select</option>
																		@foreach($customer as $row)
																			<option value="{{ $row->id }}" @if($row->id == $user->customer_id) selected @endif>{{ ucwords($row->name) }}</option>
																		@endforeach
																</select>
																<div class="fv-plugins-message-container text-danger"></div>
															</div>
